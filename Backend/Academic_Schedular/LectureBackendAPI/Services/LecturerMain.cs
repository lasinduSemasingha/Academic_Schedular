...
namespace LectureBackendAPI.Services.Lecturer
{
    public class LecturerMain : ILecturerService
    {
        ...
        public async Task<LecturerEntity> GetSingleLecturer(int id)
        {
            //Database connection creating
            using var connection = _context.Create();
            const string query = "SELECT * FROM Academic_Schedular_Lecturer WHERE lId = @id";
        ...
        {
            using var connection = _context.Create();
            const string query = "UPDATE Academic_Schedular_Lecturer SET name = @name, email = @email, phonenumber = @phonenumber, department = @department, description = @description WHERE lId = @id ";
            int rowsAffected = await connection.ExecuteAsync(query, new
            {
                id,
                name = request.name,
        ...
        {
            using var connection = _context.Create();
            const string query = "DELETE FROM Academic_Schedular_Lecturer WHERE lId = @id";
